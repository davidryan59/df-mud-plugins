...
// Custom overrides for DF Archon settings
// 1 = Normal Planet
// 2 = Asteroid Field
// 3 = Foundry
// 4 = Spacetime Rip
...
const DEFAULT_RANGE_MAX = RANGES[9][0];
...
const DEFAULT_LEVEL_MIN = 3; // default minimum level of planets to highlight
...
const periodMsHighlightSilver = 1321;
...
const colsHighlightSilver = [220, 220, 255];
...
const distanceAndLevelInRange = (plugin, planet) =>
    levelInRange(plugin, planet) && distanceInRange(plugin, planet);
...
const planetSilverHigh = (planet) => (planet.silverCap - planet.silver) / planet.silverCap < 0.19
const filter2xStat =
    (statIdx, upgradeIdx = -1) =>
    (plugin, planet) =>
        distanceAndLevelInRange(plugin, planet) &&
        planetTypeMatches(plugin, planet) &&
        ((planet.bonus && planet.bonus[statIdx]) ||
            (planet.upgradeState && planet.upgradeState[upgradeIdx] && plugin.drawOptions.show2x.value));
...
const filterSilver = (plugin, planet) =>
    distanceAndLevelInRange(plugin, planet) &&
    planetSilverHigh(planet)
const filterRip = (plugin, planet) =>
    distanceAndLevelInRange(plugin, planet) &&
    planet.planetType === SPACETIME_RIP;
    ...
    if (isToggledOn(planetArr)) {
        for (let planet of planetArr) {
            ...
            const extraRadiusFactor =
                1 +
                EXTRA_RADIUS_WORLD_PX_PULSE_MULTIPLY *
                    getTriWave01("pulseRadius");
            if (!drawOptions.line.value) {
                ctx.fillStyle = `rgba(${"" + colRgbArr[0]}, ${
                    "" + colRgbArr[1]
                }, ${"" + colRgbArr[2]}, ${"" + colAlpha})`;
            } else {
                ctx.strokeStyle = `rgba(${"" + colRgbArr[0]}, ${
                    "" + colRgbArr[1]
                }, ${"" + colRgbArr[2]}, ${"" + colAlpha})`;
            ...
            const cR =
                extraRadiusFactor *
                (viewport.worldToCanvasDist(radius) +
                    EXTRA_RADIUS_CANVAS_PX_ADD);
            const rotationRad =
                CIRC_END_RAD *
                getSawWave01(ALWAYS_PULSE, 0, PULSES_PER_ROTATION);
...
class Plugin {
    constructor() {
        ...
        this.drawOptions = {
            ellipse: { value: true, label: "Ellipse" },
            pulseOpacity: { value: false, label: "Pulse Opacity" },
            line: { value: true, label: "Line" },
            pulseRadius: { value: false, label: "Pulse Radius" },
            lineDashed: { value: true, label: "Line Dashed" },
            pulseLineWidth: { value: false, label: "Pulse Line Width" },
            sync: { value: false, label: "Sync Pulses" },
            pulseFast: { value: false, label: "Pulse Fast" },
            show2x: { value: false, label: "Show 2x" },
        };
        ...
        this.highlightData = {
            ...
            planetsWithSilver: {
                ...
                filter: filterSilver,
                array: [TOGGLE_OFF],
                periodMs: periodMsHighlightSilver,
                cols: colsHighlightSilver,
            },
            planetsWithArtifact: {
                label: "Artifacts",
