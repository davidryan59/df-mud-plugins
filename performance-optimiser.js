    ...
    const requestAnimationFrame = (callback) => {
      ...
      const loop = () => {
        ...
        if (this.rendering === "fps") {
          ...
          if (
            this.renderingFps > 0 &&
            now - lastRenderStart >= 1000 / this.renderingFps
          ) {
            lastRenderStart = now;
            return execute();
          }
          ...
          if (
            ...
            now - lastRenderStart >=
              Math.min(
                (lastRenderDuration * 100) / this.renderingPercentage,
                1000
              )
          ) {
            lastRenderStart = now;
  ...
  async render(div) {
    $(div)
      ...
      .append(
        $("<div />")
          .append(
            $('<input type="radio" name="rendering" value="unlimited" />')
          )
      ...
      .append(
        $("<div />")
          ...
          .append(
            $(`<span id="percentageSpan"> ${state.renderingPercentage}%</span>`)
          )
      )
      .append(
        $("<div />")
      ...
      .append(
        $("<div />")
          ...
          .append(
            $(`<span id="cacheLimitSizeSpan"> ${state.cacheLimit} </span>`)
          )
      )
      .append(
        $("<div />").append(
    ...
    $(`input:radio[name=rendering][value=${state.rendering}]`).prop(
      "checked",
      true
    );
